 ...
 * Usage: npx tsx scripts/delete-company.ts <companyName>
 */
import postgres from "postgres";
import { drizzle } from "drizzle-orm/postgres-js";
import * as schema from "@paperclipai/db/schema";
import { eq } from "drizzle-orm";
import { resolveDatabaseTarget } from "@paperclipai/db/src/runtime-config.js";

async function main() {
  const companyName = process.argv[2];
  if (!companyName) {
    console.error("Usage: npx tsx scripts/delete-company.ts <companyName>");
    process.exit(1);
  }
